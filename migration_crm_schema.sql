-- MIGRATION SCRIPT: Drop old tables and recreate with CRM features
-- ⚠️ WARNING: This will DELETE all existing data!
-- Use this only if you don't have important data yet, or if you've backed it up.

-- Step 1: Drop all existing tables (CASCADE removes dependencies)
DROP TABLE IF EXISTS user_playlists CASCADE;
DROP TABLE IF EXISTS rewards CASCADE;
DROP TABLE IF EXISTS scans CASCADE;
DROP TABLE IF EXISTS coffee_purchases CASCADE;
DROP TABLE IF EXISTS songs CASCADE;
DROP TABLE IF EXISTS users CASCADE;

-- Step 2: Recreate tables with new CRM schema
-- (Aici urmează schema completă)
